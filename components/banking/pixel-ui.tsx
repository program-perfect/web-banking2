...
export function PixelButton({ className, children, disabled, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      ...
      data-cursor={disabled ? "disabled" : "pointer"}
      disabled={disabled}
      {...props}
    >
      {children}
...
export function PixelInput({ className, disabled, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "h-11 w-full border-2 border-foreground bg-secondary px-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:bg-card disabled:opacity-50",
        className,
      )}
      data-cursor={disabled ? "disabled" : "text"}
      disabled={disabled}
      {...props}
    />
  )
...
export function PixelLoader({
  ...
  variant?: "bars" | "coin" | "dots" | "card" | "orbit"
  label?: string
  className?: string
}) {
  if (variant === "coin" || variant === "card") {
    return (
      <div className={cn("flex flex-col items-center justify-center gap-4 text-center", className)} role="status" aria-label={label ?? "Loading"} data-cursor="wait">
  ...
  if (variant === "orbit") {
    return (
      <div className={cn("flex flex-col items-center justify-center gap-4 text-center", className)} role="status" aria-label={label ?? "Loading"} data-cursor="wait">
        <div className="pixel-loader--orbit" aria-hidden>
          <span />
          <span />
          <span />
          <span />
  ...
  if (variant === "dots") {
    return (
      <div className={cn("inline-flex items-center gap-3", className)} role="status" aria-label={label ?? "Loading"} data-cursor="wait">
  ...
  return (
    <PixelCard aria-busy="true">
      <div className="space-y-4">
        <PixelLoader variant={tall ? "card" : "dots"} label="Loading" />
